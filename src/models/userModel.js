...
const UserModel = {
    ...
    createUser: async (user) => {
        try {
            ...
            const result = await client.query(
                "INSERT INTO users (id, nama, email, password) VALUES ($1, $2, $3, $4) RETURNING *",
                [nextId, user.nama, user.email, user.hashingPassword]
            );
    ...
    updateUser: async (id, user) => {
        const result = await client.query("UPDATE users SET nama = $1, email = $2, password = $3 WHERE id = $4 RETURNING *", [user.nama, user.email, user.password, id]);
        return result.rows[0];
    },
    deleteUser: async (id) => {
