...
const createUser = async (req , res) => {
    try {
        ...
        const hashingPassword = await bycrpt.hash(password, 10)
        const data = await UserModel.createUser({nama , email , password: hashingPassword})
        ...
        if (error.code === "23505") {
            return res.status(409).json({
                ...
                // jadi kalo semisal bikin data/tambahin data, kalo email sama baka error
            })
        }
...
const updateUser = async (req, res) => {
    try {
        const { id } = req.params
        const { nama , email, password } = req.body
        const data = await UserModel.updateUser(id, { nama , email, password })
        if (!data) {
            return res.status(404).json({
                "status": 404,
                "message": "Data User Tidak Ditemukan"
